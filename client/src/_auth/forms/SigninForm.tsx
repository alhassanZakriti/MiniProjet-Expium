...
const SigninForm = () => {
  ...
  const makeBorder = () => {
    ...
    input?.setAttribute('style','border: 2px solid #00000860')
  }
  ...
  return (
    <div className="hero-left">
      ...
      <form onSubmit={handleSubmit(submitForm)} className="auth-form">
        ...
        <div id="pw1" className="input-area">
          <input onFocus={makeBorder()} onBlur={deleteBorder()} type={isVisible? "password": "text"} className="pw" placeholder="Password" onChange={handleChange}/>
          <a onClick={()=>setIsVisible(!isVisible)}>
            {isVisible? <svg xmlns="http://www.w3.org/2000/svg" width="22" height="16" viewBox="0 0 22 16" fill="none">
